...
private fun dpTopDown(arr: Array<Int>, i: Int, cache: MutableMap<Int, Int>): Int {
    ...
    cache[i] = max(dpTopDown(arr, i - 1, cache), dpTopDown(arr, i - 2, cache) + arr[i])
    return cache[i]!!
}
...
private fun dpTable(arr: Array<Int>): Int {
    ...
    cache.put(0, arr[0])
    cache.put(1, max(arr[0], arr[1]))

    for (i in 2..arr.size - 1)
        cache.put(i, max(cache.get(i - 1)!!, cache.get(i - 2)!! + arr[i]))

    return cache.get(arr.size - 1)!!
}

fun main() {
    assert(recursion(arrayOf(2,7,9,3,1), 4) == 12)
    assert(dpTable(arrayOf(2,7,9,3,1)) == 12)
    assert(dpTopDown(arrayOf(2,7,9,3,1), 4, mutableMapOf()) == 12)
}